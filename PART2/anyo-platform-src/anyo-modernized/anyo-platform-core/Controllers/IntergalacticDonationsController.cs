...
namespace anyo_platform_core.Controllers
{
    ...
    public class IntergalacticDonationsController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> MicroserviceCreate([Bind("Message,Quantity,MissionId,DonorName,PackageId")] IntergalacticDonation intergalacticDonation)
        {
            if (ModelState.IsValid)
            {
                var url = "https://localhost:7704/Donations/Create";
                using (var client = new HttpClient())
                {
                    var json = JsonConvert.SerializeObject(intergalacticDonation);
